...
同一套 Flutter 代码构建 Android APK 与 iPhone 可添加到主屏幕的 Web App。当前已包含首页、每日问答、点滴、心情、通知中心、个人中心、相册、纪念日、共同任务、点菜、甜蜜积分、愿望、时光胶囊、默契测试、感谢墙、回忆时间轴、健康记录、双人挑战、周报与月报。扩展功能统一收纳在底部“一起”页；创建型请求带客户端请求 ID，弱网重试不会重复入库。核心内容会缓存最近一页并保留文字草稿，不持久化待上传图片。
